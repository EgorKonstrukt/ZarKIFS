...
VERT_SHADER = (_SHADER_DIR / "shaders/fractal/fractal_vert.glsl").read_text(encoding="utf-8")

FRAG_SHADER = (_SHADER_DIR / "shaders/fractal/fractal_frag.glsl").read_text(encoding="utf-8")

from post_process import PostProcessor

DEBUG_OVERLAY_VERT = (_SHADER_DIR / "shaders/debug/debug_overlay_vert.glsl").read_text(encoding="utf-8")

DEBUG_OVERLAY_FRAG = (_SHADER_DIR / "shaders/debug/debug_overlay_frag.glsl").read_text(encoding="utf-8")
...
_FRACTAL_FIELDS = [
    ...
    'fsr2_rcas_sharpness', 'fsr2_reactive_scale', 'fsr2_blend_alpha', 'fsr2_preset',
]
...
class FractalWindow(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self._post_uloc = {}
        self.post_vao  = None
        from post_process import FSR2Renderer
        ...
        self._fsr2 = FSR2Renderer(self.ctx, vbo)
        self._fsr2.set_rcas_sharpness(_params.fsr2_rcas_sharpness)
        self._fsr2.set_reactive_scale(_params.fsr2_reactive_scale)
        self._fsr2.set_blend_alpha(_params.fsr2_blend_alpha)
    ...
    def _set_shared_uniforms(self, p, elapsed, fwd, right, up, render_pos):
        ...
        _sv('u_aa_samples',     min(p.aa_samples, 3) if p.aa_samples not in (5, 6, 7) else 1)
        if p.aa_samples == 5:
            ...
            gl_wnd = _gl_window_ref[0]
            if gl_wnd is not None and hasattr(gl_wnd, '_fsr2'):
                ww, wh = gl_wnd.wnd.size
                jx, jy = gl_wnd._fsr2.get_jitter(ww, wh)
                _sv('u_taa_jitter', (jx, jy))
            else:
                _sv('u_taa_jitter', (0.0, 0.0))
        else:
    ...
    def _get_render_size(self):
        ...
        elif _params.aa_samples == 7:
            fsr2_scale = self._fsr2._scale if hasattr(self, '_fsr2') else 0.667
            rw = max(1, int(ww * scale * fsr2_scale))
            rh = max(1, int(wh * scale * fsr2_scale))
        else:
    ...
    def on_render(self, t, ft):
        ...
        if self._title_accum >= self._title_interval:
            ...
            if _params.aa_samples == 6:
                ...
                res_str = f' | {ww}x{wh} (FSR1 {_irw}x{_irh})'
            elif _params.aa_samples == 7:
                fsr2_scale = self._fsr2._scale if hasattr(self, '_fsr2') else 0.667
                _irw = max(1, int(rw * fsr2_scale))
                _irh = max(1, int(rh * fsr2_scale))
                res_str = f' | {ww}x{wh} (FSR2 {_irw}x{_irh})'
            else:
...
class ControlGUI(QMainWindow):
    ...
    def _build_aa_section(self):
        ...
        for i, lbl in enumerate(["Off (1x)", "RGSS 4x", "Grid 9x", "FXAA", "TAA", "FSR 1.0", "FSR 2.0"]):
            rb = QRadioButton(lbl)
        ...
        from post_process import FSR2_QUALITY_PRESETS, FSR2_RCAS_SHARPNESS_DEFAULT

        fsr2_grp = _section("FSR 2.0 SETTINGS")
        fsr2_l = QVBoxLayout(fsr2_grp)
        fsr2_l.setSpacing(4)
        _lbl_hint(fsr2_l, "AMD FSR 2.0: temporal upscaler — renders at reduced resolution and reconstructs full-res via temporal accumulation with variance clipping and reactive mask. Includes RCAS sharpening pass.")

        fsr2_preset_row_w = QWidget()
        fsr2_preset_row_l = QHBoxLayout(fsr2_preset_row_w)
        fsr2_preset_row_l.setContentsMargins(4, 2, 4, 2)
        fsr2_preset_row_l.setSpacing(4)
        fsr2_preset_lbl = QLabel("Quality Preset")
        fsr2_preset_lbl.setFixedWidth(90)
        fsr2_preset_lbl.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        fsr2_preset_row_l.addWidget(fsr2_preset_lbl)
        self._fsr2_preset_grp = QButtonGroup(self)
        _FSR2_PRESET_ORDER = ["Ultra Quality", "Quality", "Balanced", "Performance"]
        _FSR2_PRESET_SHORT = ["Ultra Q", "Quality", "Balanced", "Perf"]
        for pi, (pname, pshort) in enumerate(zip(_FSR2_PRESET_ORDER, _FSR2_PRESET_SHORT)):
            pb2 = QRadioButton(pshort)
            pb2.setToolTip(f"{pname}  —  {int(FSR2_QUALITY_PRESETS[pname]['scale']*100)}% render scale")
            pb2.setChecked(pname == _params.fsr2_preset)
            pb2.setProperty('preset_name', pname)
            self._fsr2_preset_grp.addButton(pb2, pi)
            fsr2_preset_row_l.addWidget(pb2)
        def _on_fsr2_preset_btn(btn):
            pname = btn.property('preset_name')
            if pname is None:
                return
            p = FSR2_QUALITY_PRESETS[pname]
            _params.fsr2_preset         = pname
            _params.fsr2_rcas_sharpness = p['rcas']
            _params.fsr2_reactive_scale = p['reactive']
            gl_wnd = _gl_window_ref[0]
            if gl_wnd is not None and hasattr(gl_wnd, '_fsr2'):
                gl_wnd._fsr2.apply_preset(pname)
            if hasattr(self, '_fsr2_rcas_row'):
                self._fsr2_rcas_row.set_value(p['rcas'])
            if hasattr(self, '_fsr2_reactive_row'):
                self._fsr2_reactive_row.set_value(p['reactive'])
            if hasattr(self, '_fsr2_info_lbl'):
                self._fsr2_info_lbl.setText(
                    f"Render: {int(p['scale']*100)}%  |  RCAS: {p['rcas']:.2f}  |  Reactive: {p['reactive']:.1f}")
        self._fsr2_preset_grp.buttonClicked.connect(_on_fsr2_preset_btn)
        fsr2_l.addWidget(fsr2_preset_row_w)

        fsr2_rcas_row = SliderRow("RCAS Sharpness", 0.0, 1.0, _params.fsr2_rcas_sharpness, 0.01)
        fsr2_rcas_row.setToolTip(
            "Robust Contrast-Adaptive Sharpening applied after temporal reconstruction.\n"
            "Higher = more sharpening on final output.")
        def _on_fsr2_rcas(v):
            _params.fsr2_rcas_sharpness = v
            gl_wnd = _gl_window_ref[0]
            if gl_wnd is not None and hasattr(gl_wnd, '_fsr2'):
                gl_wnd._fsr2.set_rcas_sharpness(v)
            if hasattr(self, '_fsr2_info_lbl'):
                self._fsr2_info_lbl.setText(
                    f"Render: {int(_params.fsr2_reactive_scale*100)}%  |  RCAS: {v:.2f}  |  Reactive: {_params.fsr2_reactive_scale:.1f}")
        fsr2_rcas_row.on_change(_on_fsr2_rcas)
        fsr2_l.addWidget(fsr2_rcas_row)
        self._fsr2_rcas_row = fsr2_rcas_row

        fsr2_reactive_row = SliderRow("Reactive Scale", 0.5, 5.0, _params.fsr2_reactive_scale, 0.1)
        fsr2_reactive_row.setToolTip(
            "Controls how aggressively high-contrast areas (particles, specular) are treated as reactive.\n"
            "Higher = less ghosting on dynamic elements, more noise.")
        def _on_fsr2_reactive(v):
            _params.fsr2_reactive_scale = v
            gl_wnd = _gl_window_ref[0]
            if gl_wnd is not None and hasattr(gl_wnd, '_fsr2'):
                gl_wnd._fsr2.set_reactive_scale(v)
        fsr2_reactive_row.on_change(_on_fsr2_reactive)
        fsr2_l.addWidget(fsr2_reactive_row)
        self._fsr2_reactive_row = fsr2_reactive_row

        fsr2_blend_row = SliderRow("Blend Alpha", 0.05, 0.5, _params.fsr2_blend_alpha, 0.005)
        fsr2_blend_row.setToolTip(
            "Weight of the current frame in temporal accumulation.\n"
            "Lower = smoother, more ghosting. Higher = sharper, less stable.")
        def _on_fsr2_blend(v):
            _params.fsr2_blend_alpha = v
            gl_wnd = _gl_window_ref[0]
            if gl_wnd is not None and hasattr(gl_wnd, '_fsr2'):
                gl_wnd._fsr2.set_blend_alpha(v)
        fsr2_blend_row.on_change(_on_fsr2_blend)
        fsr2_l.addWidget(fsr2_blend_row)
        self._fsr2_blend_row = fsr2_blend_row

        fsr2_info_lbl = QLabel(
            f"Render: {int(FSR2_QUALITY_PRESETS.get(_params.fsr2_preset, FSR2_QUALITY_PRESETS['Quality'])['scale']*100)}%"
            f"  |  RCAS: {_params.fsr2_rcas_sharpness:.2f}"
            f"  |  Reactive: {_params.fsr2_reactive_scale:.1f}")
        fsr2_info_lbl.setStyleSheet("color: #8888cc; font-size: 10px; padding: 2px 6px;")
        fsr2_l.addWidget(fsr2_info_lbl)
        self._fsr2_info_lbl = fsr2_info_lbl

        fsr2_grp.setVisible(_params.aa_samples == 7)
        self._fsr2_settings_grp = fsr2_grp
        layout.addWidget(fsr2_grp)
    ...
    def _on_aa_changed(self, idx: int):
        ...
        if hasattr(self, '_fsr2_settings_grp'):
            self._fsr2_settings_grp.setVisible(idx == 7)
        gl_wnd = _gl_window_ref[0]
        if gl_wnd is not None and hasattr(gl_wnd, '_post'):
